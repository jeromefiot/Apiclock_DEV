...
from ..my_podcast import ApiPodcast
...
def podcast(action):
    ...
    if action == "unsubscribe":
        ...
        try:
            ...
            url_file = (os.path.join(app.config['BASE_APP_DIR'], app.config['UPLOAD_FOLDER'], filename))
        ...
        if file.filename:
            ...
            # add try / except for file already existing
            filename = 'UPLOAD_'+str(filename)
            app = current_app._get_current_object()

            file_path = os.path.join(app.config['BASE_APP_DIR'],
                                     app.config['UPLOAD_FOLDER'],
                                     filename)
            file.save(file_path)
            file_url = os.path.join(app.config['BASE_APP_DIR'], app.config['UPLOAD_FOLDER'], filename)
            addmusic2 = Music(
                    name=filename,
                    url=file_url,
                    music_type=3,
...
def testpodcast():
    ...
    temp = []
    dictlist = []
    zob = testp.infospodcast(url='http://radiofrance-podcast.net/podcast09/rss_10076.xml', max_episodes=5)
    for key, value in zob.iteritems():
        temp = [key, value]
